  ...
  <q-card @changeState="onMounted()">
        ...
        <q-tab-panels v-model="tab" animated>
          <q-tab-panel name="history">
            <q-table title="Historial de permisos" :rows="row" :columns="column">
            </q-table>
          </q-tab-panel>
      ...
      api.get('users/permises')
        .then(res => {
          ...
          for (let i = 0; i < tableData.value.length; i++) {
            const {fecha_salida,fecha_llegada,hora_salida,hora_llegada,tipo,estado,usado,lugar} = tableData.value[i]
